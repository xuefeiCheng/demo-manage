  ...
  <div class="div-border">
    ...
    <Modal v-model="userModal" title="新增用户" :loading="saveLoading"  @on-ok="save" @on-cancel="cancel()">
      ...
      <Form ref="user" :model="user" :label-width="100" :rules="userRules">
        <FormItem label="菜单名称" prop="userName">
          <Input v-model="user.userName" placeholder="请输入菜单名称..."></Input>
        </FormItem>
        <FormItem label="所属菜单" prop="department">
          <Input v-model="user.department" placeholder="请输入所属菜单..."></Input>
        </FormItem>
      </Form>
    </Modal>
...
<script>
import {menu} from 'static/data/staticdata.js'
export default {
  data () {
    return {
      search: {
