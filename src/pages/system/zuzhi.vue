  ...
  <div class="div-border">
    ...
    <Modal v-model="userModal" title="新增" :loading="saveLoading"  @on-ok="save" @on-cancel="cancel()">
      ...
      <Form ref="user" :model="user" :label-width="100" :rules="userRules">
        <FormItem label="组织名称" prop="userName">
          <Input v-model="user.userName" placeholder="请输入组织名称..."></Input>
        </FormItem>
        <FormItem label="所属机构" prop="department">
          <Input v-model="user.department" placeholder="请输入所属机构..."></Input>
        </FormItem>
      </Form>
    </Modal>
...
<script>
import {zuzhi} from 'static/data/staticdata.js'
export default {
  data () {
    return {
      search: {
