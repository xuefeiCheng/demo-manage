  ...
  <div class="div-border">
    ...
    <Form :model="search" :label-width="150">
      <Row>
        <Col span="8">
          <FormItem label="用户名">
            <Input v-model="search.userName" placeholder="请输入要查询的用户名..."></Input>
          </FormItem>
        </Col>
        <Col span="8">
          <FormItem label="创建时间">
    ...
    <!-- 新增用户 -->
    <Modal v-model="userModal" title="新增用户" :loading="saveLoading"  @on-ok="save" @on-cancel="cancel()">
      ...
      <Form ref="user" :model="user" :label-width="100" :rules="userRules">
        ...
        <FormItem label="密码有效期" prop="validity">
          <InputNumber v-model="user.validity" placeholder="请输入密码有效期..." :formatter="formatValidity"
            :parser="parserValidity" style="width: 100%;"></InputNumber>
        </FormItem>
        <FormItem label="真实姓名" prop="realName">
          <Input v-model="user.realName" placeholder="请输入真实姓名..."></Input>
        </FormItem>
        <FormItem label="所属部门" prop="department">
          <Input v-model="user.department" placeholder="请输入所属部门..."></Input>
        </FormItem>
        <FormItem label="手机号码" prop="phone">
          <Input v-model="user.phone" placeholder="请输入手机号码..."></Input>
        </FormItem>
      </Form>
    </Modal>
          ...
          render: (h, params) => {
            return h('div', [
              h('Button', {
                ...
                on: {
                  click: () => {
                    // let userNumber = params.row.userName
                  }
                }
              }, '角色详情'),
              h('Button', {
              ...
              h('Button', {
                ...
                on: {
                  click: () => {
                    this.invalid(params.row.userName)
                  }
                }
              }, '注销')
            ])
