          ...
          <div class="box_conent border-self">
            <div class="inter_table">
              <div class="inter_table_header">
                <ul>
                  <li style="width:18%">手机号</li>
                  <li>运营商</li>
                  <li>事件</li>
                  <li style="width:40%">事件详情</li>
                ...
                <div class="inter_table_body" style="height:90%">
                  <div :class="{anim: animate === true}">
                    <ul v-for="(item, index) in tableData" :key="index">
                      <li :title='item.real_msisdn' class="YELLOW" style="width:18%">{{item.real_msisdn | formatNum6}}</li>
                      <li>{{item.user_comcode}}</li>
                      <li class="YELLOW">{{item.charac_subcategory}}</li>
                      <li :title='item.charac_category_detail' style="width:40%">{{item.charac_category_detail | substr(16,'...')}}</li>
                      <li :title='item.commit_time' class="YELLOW" style="width:12%">{{item.commit_time | substr(8,'')}}</li>
                    </ul>
                  </div>
                </div>
              </transition>
                ...
                <div class="inter_table_body" style="height:80%">
                  <div :class="{anim: animate === true}">
                    <ul v-for="(item, index) in safeDate" :key="index">
                      ...
                      <li style="width:30%">{{item.dst_ip}}</li>
                    </ul>
                  </div>
                </div>
              </transition>
      ...
      axios.get('interbigscreen/getydmaliceCount')
        .then((res) => {
          if (res.code === 200) {
            ...
            this.drawSpecial('bottom_4', this.Ydmalice.legend, this.Ydmalice.data, '事件', '移动恶意事件类型次数统计', {radius: ['35%', '55%'], center: ['45%', '60%']}, '事件总数', 'big')
          }
        })
    },
    drawSpecial (ID, legend, data, name, title, position, text, titltStyle) {
