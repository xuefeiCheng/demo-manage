<template>
  <div class="data noback mapTab map_margin">
    ...
    <div class="data-content" ref="dataContent" style="height:100%">
      ...
      <Modal v-model="gxModal" footer-hide :closable="false" class="wjzModal" :width="1100">
        ...
        <div id="gxMap"  class="hideLogo" style="width: 1090px; height:400px"></div>
      </Modal>
...
export default {
  ...
  methods: {
    ...
    drawHuan () {
      ...
      axios.get('bigscreen/getSpamhour')
        .then(
          (res) => {
            if (res.code === 200) {
    ...
    drawMap () { // 基于准备好的dom，初始化echarts实例
      ...
      this.setmapIntervalid = setInterval(() => {
        minigrateChart.clear()
        minigrateChart.setOption(MinigrateOption)
        if (_this.actionindex === 0) {
          _this.$store.dispatch('fetchGxData', minigrateChart)
          _this.actionindex = 1
        } else {
          _this.$store.dispatch('fetchHeatChinaRealData', minigrateChart)
          _this.actionindex = 0
