    ...
    <div class="data-content2">
      <div class="all fl">
        <div class="info">
          ...
          <div class="info-concent charts" id="small_echarts_2"></div>
        </div>
      </div>
      <!-- 百度地图 -->
      <!-- :fullscreen = 'fullscreenF'  draggable-->
      <Modal v-model="smallWjzMap" footer-hide :closable="false" class="wjzModal" :width='1100'>
...
export default {
  ...
  methods: {
    ...
    drawGuangxi () { // 基于准备好的dom，初始化echarts实例
      let _this = this
      let minigrateChart = this.$echarts.init(document.getElementById('small_echarts_2'))
      let MinigrateOption = {
        tooltip: {
          trigger: 'item',
          formatter: '{a} <br/>{b} : {c}'
      ...
      axios.get('bigscreen/getAllPaseudo')
        .then((res) => {
          if (res.code === 200) {
            let result = res.data
            this.pointsSource = []
    ...
    closeModal () {
      this.smallWjzMap = false
      this.map = null
      const content = this.$refs.small_baiduMap
      content.removeChild(content.childNodes[1])
      // this.$refs.small_baiduMap.empty()
    }
  },
  beforeDestroy () {
    // 清除定时器
