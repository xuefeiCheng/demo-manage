    ...
    <div class="inter_top_box">
      <div class="inter_top_boxL">
        <div class="inter_TBL_box_top">
          ...
          <div class="inter_TBL_box_top_item">
            <div class="box_title">蠕虫事件总数</div>
                ...
                <div class="inter_table_body">
                  <div :class="{anim: animate === true}">
                    <ul v-for="(item, index) in tableData" :key="index">
                      ...
                      <li class="BLUE">{{item.user_comcode}}</li>
                      <li class="ORANGE">{{item.charac_subcategory}}</li>
                      <li :title='item.charac_category_detail'>{{item.charac_category_detail | substr(12,'...')}}</li>
                      <li :title='item.commit_time' class="YELLOW">{{item.commit_time | substr(8,'')}}</li>
                    </ul>
                  </div>
                </div>
              </transition>
                ...
                <div class="inter_table_body">
                  <div :class="{anim: animate === true}">
                    <ul v-for="(item, index) in safeDate" :key="index">
                      ...
                      <li class="BLUE" style="width:30%">{{item.primary_class}}</li>
                      <li class="ORANGE" style="width:30%">{{item.dst_ip}}</li>
                    </ul>
                  </div>
                </div>
              </transition>
...
export default {
  ...
  mounted () {
    ...
    this.$nextTick(() => {
      ...
      // 数据表格 高度 [新加]
      const tableDataHeight = document.getElementsByClassName('inter_table')[0].offsetHeight
      const tableDataHearderH = document.getElementsByClassName('inter_table_header')[0].offsetHeight
      const tableDatacontent = document.getElementsByClassName('inter_table_body')
      tableDatacontent[0].style.height = tableDataHeight - tableDataHearderH - 25 + 'px'
      // 减去 padding值
  ...
  methods: {
      ...
      axios.get('interbigscreen/geteventtread')
        .then((res) => {
          if (res.code === 200) {
            ...
            this.drawBottom_23('bottom_3', '事件用户数变化趋势统计（周）', result.date, result.dyuser, result.wfuser)
          }
        })
    },
    jmrtrend () {
      ...
      axios.get('interbigscreen/getydmaliceCount')
        .then((res) => {
          if (res.code === 200) {
            ...
            this.drawMapByType('bottom_4', this.Ydmalice.legend, this.Ydmalice.data, '事件', '移动恶意事件类型次数统计', {radius: ['35%', '55%'], center: ['45%', '60%']}, '事件总数', 'big')
          }
        })
    },
    // 恶意程序8大类
      ...
      axios.get('interbigscreen/getmalice')
        .then((res) => {
          if (res.code === 200) {
            ...
            this.drawMapByType('bottom_5', this.Malice.legend, this.Malice.data, '事件', '恶意程序八大类占比图', {radius: ['35%', '55%'], center: ['45%', '60%']}, '事件总数', 'big')
          }
        })
    },
    // 受害运营商周统计
      ...
      axios.get('interbigscreen/getweekarea')
        .then((res) => {
          if (res.code === 200) {
            ...
            this.drawMapByArea('chinaMap', 'china', chinadate, {show: false, title: '', mapTitle: '区域分布周统计'}, 'china')
          }
        })
    },
    // 分布区域周统计
    jmarea () {
      axios.get('interbigscreen/getjmarea')
        .then((res) => {
          if (res.code === 200) {
            let gxdate = res.data
            this.drawMapByArea('GxMap', '广西', gxdate, {show: false, title: '', mapTitle: '僵尸木马事件地域分布图'}, '广西')
          }
        })
    },
    // 诈骗源国家分布周统计
    weekcountry () {
      ...
      axios.get('interbigscreen/getjmr')
        .then((res) => {
          if (res.code === 200) {
            let result = res.data
            this.drawworldMap('worldMap', this.worlddate, '诈骗源国家分布周统计', result)
          }
        }) */
      // this.drawworldMap('worldMap', '诈骗源国家分布周统计')
      let charts = this.$echarts.init(document.getElementById('worldMap'))
      charts.clear()
      let options = {
        title: {
          show: true,
    ...
    drawMapByArea (ID, Areaname, data, others, flag) {
      ...
      minigrateChart.on('click', function (params) {
        ...
        if (flag) {
          if (flag === '广西') {
            _this.drawMapByArea('BigMap', '广西', data, {show: false, mapTitle: '僵尸木马事件地域分布图', symbolSize: 15})
          } else if (flag === 'china') {
            _this.drawMapByArea('BigMap', 'china', data, {show: false, mapTitle: '区域分布周统计', symbolSize: 15})
          }
        }
        setTimeout(() => {
    ...
    drawworldMap (ID, title) {
      let _this = this
      let worldChart = this.$echarts.init(document.getElementById(ID))
      worldChart.clear()
        ...
        tooltip: {
          ...
          formatter: function (params) {
            if (params.componentSubType === 'lines') {
              return params.data.fromName + '> ' + params.data.toName + ' ' + params.data.numValue + '条'
      ...
      // _this.createseries(worlddata, dd, worldChart)
      window.addEventListener('resize', function () {
        worldChart.resize()
      })

      _this.$store.dispatch('fetchworldData', worldChart)

      worldChart.on('click', function (params) {
        _this.mapTitle = '诈骗源国家分布周统计'
        _this.drawworldMap('BigMap', '诈骗源国家分布周统计')
        setTimeout(() => {
          _this.baiduModal = true
        }, 20)
      })
